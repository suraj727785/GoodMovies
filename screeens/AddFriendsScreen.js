      ...
      for (var i = 0; i < addFriendsList.length; i++) {
        for (var j = 0; j < data.length; j++) {
          if (data[j].phoneNumbers) {
            if (
              addFriendsList[i].mobileNo
                .replace(/\s+/g, "")
                .includes(data[j].phoneNumbers[0].number.replace(/\s+/g, ""))
            ) {
      ...
      {users === undefined ? (
        ...
        <View style={styles.noFriendsShowView}>
          <Text style={{ fontSize: 16 }}>
            You already added all your contacts who are here...
          </Text>
          <Text style={{ fontSize: 16 }}>
            Please tell more contacts to join GoodMovies
          </Text>
        </View>
      ) : (
        <FlatList
